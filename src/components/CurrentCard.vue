<template>
  <div v-if="card.location" class="card">
    <div class="card__location">
      <h1 class="card__location-name main-text">{{ card.location.name }}</h1>
      <time
        :datetime="card.location.localtime"
        class="card__location-time secondary-text"
        >Now is {{ localtimeWithoutData }}</time
      >
    </div>
    <div class="card__current">
      ...
      <img
        class="card__current-img"
        :src="card.current.condition.icon"
        alt="condition"
      />
    ...
    <div class="card__facts">
      <div class="card__facts-item">
        <img
          ...
          alt="wind"
        />
        <p class="card__facts-wind">
          {{ windSpeed }} м/с, {{ windDirName }}
          <img
            :style="windDirImage"
...
const feelsLikeTemp = computed(() =>
  formatedTemperature(props.card.current.feelslike_c)
);
const currentTemp = computed(() =>
  formatedTemperature(props.card.current.temp_c)
);
</script>
